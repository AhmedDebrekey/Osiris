...
- **8G — Play/Edit mode.** Not started — owner's design intent, captured here so it isn't lost before Phase 8 starts:
  - An `Engine`-level Play/Edit toggle, F5 (`Input::IsKeyPressed(SDL_SCANCODE_F5)`, edge-triggered).
  - **Edit mode (default)**: `SceneInspectorPanel`/ImGui visible; Dynamic/Kinematic bodies don't simulate (`IPhysics::Update` not called, so nothing falls); scripts don't run at all (`IScripting::Update`/`FixedUpdate` not called — no `OnStart`/`OnUpdate`/`OnFixedUpdate`).
  - **Play mode**: ImGui hidden (immersive view of the actual game, not the editor); physics and scripting actually step every frame; each script instance's `OnStart` fires the first time Play is entered (already lazy per-instance — see `LuaScripting::Update` — so this falls out naturally once `Update`/`FixedUpdate` are gated behind Play mode rather than always called).
  - Open question for whenever this gets built, not resolved here: whether re-entering Edit mode should snapshot/restore Dynamic body positions and script state back to how they were before Play started (the standard Unity/Unreal/Godot behavior), or just leave whatever Play mode left behind.
