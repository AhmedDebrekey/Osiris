...
1. **8G checkpoint 2** (the actual Play/Edit toggle) then **checkpoint 3** (snapshot/restore) — in progress, see Phase 8 above.
2. Rest of **7C** (interaction system, inventory) — deliberately deferred: it's more naturally scoped alongside Phase 9 (the horror prototype needs interaction anyway) than as standalone work right now. Revisit when Phase 9 gets closer, not before.
3. Round out **7B**'s remaining scope (OGG, reverb zones, occlusion) whenever audio needs more than a looping test emitter — none of it blocks anything else.
4. **8A** (docked editor) whenever panel-based iteration starts feeling cramped — `SceneInspectorPanel` already covers basic inspection (plus Add/Remove Component) in the meantime. **Bloom + SSAO** (rest of 6D) remains available too if visual polish is wanted at some point.
