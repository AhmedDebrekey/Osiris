...
namespace {
    struct LaunchConfig {
        std::string scene = "scenes/horror.json";
    ...
    LaunchConfig LoadLaunchConfig() {
        ...
        if (json.is_discarded()) {
            APP_ERROR("Failed to parse the horror game.json");
            return config;
        }
...
int main() {
    std::cout << "Osiris Horror" << std::endl;
    Osiris::Engine engine;

    WindowDesc windowDesc;
    windowDesc.title = "Osiris Horror";
    if (!engine.Initialize(windowDesc, OSIRIS_GAME_ASSET_ROOT)) {
    ...
    if (!launchConfig.environment.empty()) {
